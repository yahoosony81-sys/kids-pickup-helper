...
**3-1. Clerk Dashboard에서 Supabase Integration 활성화**
...
7. **Clerk domain** 값이 표시됩니다 (예: `your-app-12.clerk.accounts.dev`)
   - 이 값을 복사하여 메모해두세요 (다음 단계에서 사용)

> **참고**: Clerk Dashboard에서 Supabase integration을 활성화하면, Clerk 세션 토큰에 `"role": "authenticated"` JWT claim이 자동으로 추가됩니다.

**3-2. Supabase Dashboard에서 Clerk Third-Party Provider 설정**
...
5. **"Add provider"** 클릭
6. **"Clerk"** 선택
7. **"Clerk domain"** 필드에 3-1에서 복사한 Clerk domain 입력
   - 예: `your-app-12.clerk.accounts.dev`
8. **"Save"** 클릭

> **참고**: Supabase는 Clerk domain을 사용하여 자동으로 JWT Issuer URL과 JWKS Endpoint를 구성합니다.
...
2. 다음 값들을 복사하여 `.env` 파일에 입력:
   ...
   # Publishable key (새 형식, 권장) 또는 Anon key (레거시)
   # API Keys 섹션에서 둘 다 찾을 수 있습니다
...
> - Supabase는 새로운 Publishable key 형식(`sb_publishable_xxx`)을 권장합니다. 기존 anon key도 계속 사용 가능합니다.
...
**테스트 페이지:**
- `/instruments`: Supabase 공식 문서 예시 (데이터 조회)
