...
export async function expireRequestIfPast(
  ...
  try {
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/528c9e7e-7e59-428c-bfd2-4d73065ea0ec',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'request-expiration.ts:109',message:'Before update: request state',data:{requestId,currentStatus:request.status,pickupTime:request.pickup_time,requestExists:!!request},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'A'})}).catch(()=>{});
    // #endregion
    ...
    // #region agent log
    fetch('http://127.0.0.1:7242/ingest/528c9e7e-7e59-428c-bfd2-4d73065ea0ec',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'request-expiration.ts:125',message:'After update: error check',data:{hasError:!!updateError,errorType:updateError?.constructor?.name,errorKeys:updateError?Object.keys(updateError):[],errorString:String(updateError),errorJSON:updateError?JSON.stringify(updateError):null},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'B'})}).catch(()=>{});
    // #endregion

    if (updateError) {
      // #region agent log
      fetch('http://127.0.0.1:7242/ingest/528c9e7e-7e59-428c-bfd2-4d73065ea0ec',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'request-expiration.ts:120',message:'Update error details',data:{requestId,errorMessage:updateError.message,errorCode:updateError.code,errorDetails:updateError.details,errorHint:updateError.hint,errorFull:JSON.stringify(updateError,Object.getOwnPropertyNames(updateError))},timestamp:Date.now(),sessionId:'debug-session',runId:'run2',hypothesisId:'C'})}).catch(()=>{});
      // #endregion
